/**
  ******************************************************************************
  * File Name          : UTM.c
  * Description        : convert to UTM Coordinate 
...
{
	/*Waypoint_Index_1, Waypoint_Index_2 >=1 */
	/*Waypoint_Index_2 > Waypoint_Index_1*/
	double Path_Heading = 0;
//Path_Heading = RAD2DEG * atan2((dUTM_Coordinate[Waypoint_Index_2-1][1]-dUTM_Coordinate[Waypoint_Index_1-1][1]),(dUTM_Coordinate[Waypoint_Index_2-1][0]-dUTM_Coordinate[Waypoint_Index_1-1][0]));
//	if (Path_Heading < 0)
//			Path_Heading = Path_Heading + 360;
//	if (Path_Heading >=0 && Path_Heading < 90)
//			Path_Heading =  90 -Path_Heading;
//	else if (  Path_Heading >= 90 && Path_Heading < 180 )
//			Path_Heading = 270+(180-Path_Heading);
//	else if (  Path_Heading >= 180 &&  Path_Heading < 270 )
//			Path_Heading = 180 + (  270 - Path_Heading );
//	else if (  Path_Heading >= 270 &&  Path_Heading < 360 )
//			Path_Heading = 90 + ( 360 -  Path_Heading );
	
	double dx,dy =0;
	dx = dUTM_Coordinate[Waypoint_Index_2-1][0]-dUTM_Coordinate[Waypoint_Index_1-1][0];
	dy = dUTM_Coordinate[Waypoint_Index_2-1][1]-dUTM_Coordinate[Waypoint_Index_1-1][1];
	if ( dx > 0)
		Path_Heading = 90 - atan(dy/dx)*RAD2DEG;
	else if ( dx <0)
		Path_Heading = 270 -atan(dy/dx)*RAD2DEG;
	else if ( dy > 0)
		Path_Heading = 0;
	else if (dy < 0)
		Path_Heading = 180;
